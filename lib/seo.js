...
export function buildMetadata({ title, description, path = "/", keywords } = {}) {
  ...
  return {
    ...
    openGraph: {
      ...
      description: desc,
      // og:image is provided by app/opengraph-image.js (file convention) so it
      // is generated, branded, and auto-sized — no manual image entry here.
    },
    twitter: {
      card: "summary_large_image",
      title: title || `${SITE_NAME} — Modularne i montažne kuće po meri`,
      description: desc,
      // twitter:image comes from app/twitter-image.js (file convention).
    },
  };
}
